...
export async function deriveVaultPDA(vaultKind: VaultKind): Promise<[PublicKey, number]> {
    const programId = new PublicKey(getProgramId());

    const seeds = [
        Buffer.from(VAULT_SEED),
        Buffer.from(vaultKind),
    ];
...
export async function deriveVaultTokenAccountPDA(vaultPDA: PublicKey, tokenMint: PublicKey): Promise<PublicKey> {
    const TOKEN_PROGRAM_ID = new PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
    const ASSOCIATED_TOKEN_PROGRAM_ID = new PublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL");
    const [address] = PublicKey.findProgramAddressSync([vaultPDA.toBuffer(), TOKEN_PROGRAM_ID.toBuffer(), tokenMint.toBuffer()], ASSOCIATED_TOKEN_PROGRAM_ID);
    return address;
}
