...
import { PublicKey, SolanaClient, buildWithdrawalToUserTransaction } from "@repo/solana";
...
const tradesWorker = new Worker("trades", async (job) => {
...
const withdrawalsWorker = new Worker("withdrawals", async (job) => {
    ...
    try {
        const destinationAccount = getAssociatedTokenAddressSync(new PublicKey(withdrawal.tokenMint), new PublicKey(withdrawal.destinationAddress));
        ...
        console.error(`Withdrawal ${withdrawalId}: error building withdrawalToUser tx:`, error);
    }
