...
export function RecentTrades({ symbol, tokenMint }: RecentTradesProps) {
  ...
  const { data: tradesData } = useQuery({
    ...
    queryFn: () => getRecentTrades(token ?? null, { tokenMint, limit: 20 }),
          ...
          {trades.length > 0 ? (
            trades.slice(0, 20).map((trade) => {
              ...
              return (
                <div
                  key={trade.id}
                  className="grid grid-cols-3 gap-2 py-1 text-xs hover:bg-bg-tertiary rounded px-2 -mx-2"
                >
                  <div className={`font-mono flex items-center gap-1 ${side === "buy" ? "text-accent-buy" : "text-accent-sell"}`}>
                    {side === "buy" ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                    ${price.toFixed(2)}
                  </div>
                  <div className="text-right text-text-secondary font-mono">{amount.toFixed(4)}</div>
            ...
            <div className="text-center text-text-muted text-xs py-4">No recent trades</div>
          )}
        </div>
      </div>
    </div>
